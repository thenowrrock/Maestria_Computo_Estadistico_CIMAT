...
library("MASS")
#library("alr3")
library("stats") #esta libreria incluye la funcion que realiza el MDS clásico
...
# Se aplica el modelo clásico de MDS (coordenadas principales)

#función que realiza el mds clasico
...
#add: cuando  se añade una constante aditiva para convertir las disimilaridades
#en distancias euclideas
#x.ret indica si la matriz de doble centrado (B) se regresa

#Nota: si no se especifica la dimension, por default se da la solución en dos dimensiones
...
#ndim: indica la dimensionalidad de la solución considerada
#type:indica el tipo de transformación que vamos a usar para convertir las disimilaridades
#a disparidades
...
#se obtienen los diagrama de shepard, para evaluar la calidad de la solución y la calidad
...
#en los años 70, reflejando la situación socio politica de los paises en esa epoca

#la dim 1 se puede interpretar como una dimension de alineacion política o regimen politico (comunistas y capitalistas)
#la dim 2 se puede interpretar como una dimension de desarrollo económico(paises desarrollados y subdesarrollados)
...
###Aplicación de modelos de unfolding
...
#función que realiza unfolding utilizando el algoritmo SMACOF
...
#conditionality: La manera de aplicar la transformación. Se tienen dos opciones:
...
# se realiza un diagnostico para evaluar la calidad de la solución y la calidad de la transformacion
...
#se obtiene el diagrama de shepard, para evaluar la calidad de la solución, graficando
...
#el eje vertical (dim2) no tiene una clara interpretación, aunque tal vez discrimina basado en la
